...
文章点赞的在线服务配置见 [开通文章点赞](docs/article-likes.md)。
...
分享入口已恢复为 5.1（提交 `5bb2d6f`）的逻辑：“微信分享”在微信内提示使用右上角菜单；其他环境优先调用 `navigator.share({ url })`，不附加标题文本。取消分享时不复制；接口不可用或失败时保留 5.1 的复制链接备用逻辑。接收方是否生成卡片仍需手机实测，不能仅由网页参数保证。

安卓 App 单独接入系统分享：可信站点的文章按钮被点击时，通过 `ACTION_SEND` 打开系统选择面板，仅发送当前文章网址，标题用于面板预览，不再依赖 WebView 的 `navigator.share`。取消面板不会自动复制；“一键复制链接”保持独立。微信提供哪些接收入口、是否生成卡片需实机验证，此改动不保证朋友圈入口出现。普通浏览器和苹果网页分享逻辑保持原样。

App 源码变更需重新构建并安装 APK，仅更新网页不会更新原生功能。本地脚本测试 `node --test scripts/android-share.test.js` 检查按钮拦截、重复绑定及合成点击；Android 编译和真实分享面板仍需 SDK / 手机验证。
...
本地后台点击“抓取14场足彩”时，会自动读取 ODDSET 赛前全场胜平负赔率，在“指数”列按 `主胜 平 主负` 合并显示，复制表格和 Excel 接口导出保持相同格式。弹窗显示匹配数量及抓取时间，Excel 的“赔率说明”工作表记录来源、时间和抓取状态。

运行前执行 `npm install`，电脑需安装 Chrome 或 Edge。抓取使用独立的无界面浏览器，不读取个人浏览器资料。相同期次日期范围的结果缓存 60 秒，再次点击时按需更新；不会后台持续刷新。网站拒绝访问或抓取失败时，对阵仍正常显示，指数留空并提示原因。

中英文队名映射保存在 `scripts/oddset-teams.json`，目前覆盖第 26121 期的球队。后续新增球队需补充准确的 ODDSET 全名或简称。只有主客队均匹配且开赛时间相差不超过 5 分钟、结果唯一时才填入赔率，未知队名不会模糊猜测。已开赛、封盘或没有常规时间胜平负的比赛留空。

匹配与盘口过滤测试：`node --test scripts/oddset.test.js`。
