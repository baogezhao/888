...
“微信好友 / 朋友圈”按钮用于文章卡片分享引导：微信内提示使用右上角菜单；其他浏览器或 App 内提示复制链接，发到微信文件传输助手并打开文章，再使用微信菜单分享。复制需要用户主动点击，不会自动发送消息或打开微信。该按钮使用独立标识，避免旧版 App 将其拦截成系统文本分享。

安卓 App 的“更多分享”按钮通过系统分享面板选择接收应用，内容为标题与链接。原生功能更新需要重新构建并安装新版 APK，仅发布网页不会更新手机上已安装的 App。分享结果是否显示标题、图片卡片由接收应用决定。

在手机浏览器中，支持 Web Share 的浏览器会打开系统分享面板；不支持或调用失败时，网页提示使用浏览器自身的分享菜单，不会自动改成复制链接。“一键复制链接”仍可单独使用；微信内打开时仍提示使用右上角菜单分享。
